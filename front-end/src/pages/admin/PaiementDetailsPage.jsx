// pages/admin/PaiementDetailsPage.jsx
...
const typesPaiementColors = {
  'Scolarité': { bg: '#e0e7ff', color: '#4338ca' },
  'Transport': { bg: '#dcfce7', color: '#16a34a' },
  'Cantine': { bg: '#fef3c7', color: '#d97706' },
  'Activité': { bg: '#fce7f3', color: '#db2777' },
  'Matériel': { bg: '#e0f2fe', color: '#0284c7' },
  'Autre': { bg: '#f1f5f9', color: '#475569' },
};

const methodesPaiementIcons = {
  'Espèces': '💵',
  'Virement': '🏦',
  'Chèque': '📝',
  'Carte bancaire': '💳',
...
export default function PaiementDetailsPage() {
  ...
  useEffect(() => {
    ...
    if (payment) {
      ...
      // Payment not found, redirect to list
      navigate('/paiements');
    }
  ...
  const typeColor = typesPaiementColors[paiement.type] || { bg: '#f1f5f9', color: '#475569' };
  ...
  const InfoRow = ({ icon, label, value, highlight = false }) => (
    <div className="flex items-start gap-3 py-3 border-b border-slate-100 last:border-0">
      ...
      <div className="flex-1">
        ...
        <p className={`text-sm font-semibold mt-0.5 ${highlight ? 'text-orange-600' : 'text-slate-700'}`}>
          {value || '—'}
        </p>
      </div>
    </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => navigate(`/paiements/modifier/${paiement.id}`, { state: { paiement } })}
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <InfoRow icon="🎓" label="Type de paiement" value={paiement.type}>
                  <span className="text-xs font-bold px-2 py-0.5 rounded-md" style={{ background: typeColor.bg, color: typeColor.color }}>
                    {paiement.type}
                  </span>
                </InfoRow>
                <InfoRow icon="💳" label="Méthode de paiement" value={`${methodesPaiementIcons[paiement.methode]} ${paiement.methode}`} />
                <InfoRow icon="📅" label="Date d'échéance" value={formatDate(paiement.dateEcheance)} />
                <InfoRow icon="💰" label="Date de paiement" value={formatDate(paiement.datePaiement)} />
              </div>
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-xl" style={{ background: '#f8fafc' }}>
                  ...
                  <p className="text-2xl font-bold" style={{ color: '#1e3a5f' }}>
                    {paiement.montant.toLocaleString()} <span className="text-sm">DH</span>
                  </p>
                </div>
                <div className="p-4 rounded-xl" style={{ background: '#f8fafc' }}>
                  <p className="text-xs text-slate-400 mb-1">Montant payé</p>
                  <p className="text-2xl font-bold" style={{ color: '#16a34a' }}>
                    {paiement.montantPaye.toLocaleString()} <span className="text-sm">DH</span>
                  </p>
                </div>
              </div>
              ...
              {resteAPayer > 0 && (
                <div className="p-4 rounded-xl" style={{ background: '#fef2f2' }}>
                  ...
                  <p className="text-xl font-bold text-red-600">
                    {resteAPayer.toLocaleString()} <span className="text-sm">DH</span>
                  </p>
                </div>
              )}
        ...
        <div className="space-y-5">
            ...
            <div className="space-y-4">
              <div className="flex gap-3">
                <div className="relative">
                  ...
                  {paiement.datePaiement && <div className="absolute left-4 top-8 w-0.5 h-8 bg-slate-200"></div>}
                </div>
                <div className="flex-1 pb-4">
                  <p className="text-xs font-semibold text-green-600">Paiement enregistré</p>
                  <p className="text-xs text-slate-400 mt-1">{formatDate(paiement.datePaiement) || 'Date non définie'}</p>
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
            ...
            <div className="space-y-2">
              ...
              <button
                onClick={() => navigator.clipboard.writeText(paiement.reference)}
                className="w-full flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-orange-700 leading-relaxed">
                  Ce paiement a été enregistré avec la référence {paiement.reference} et le statut "{statut.label}".
                  {resteAPayer > 0 && ` Un montant de ${resteAPayer.toLocaleString()} DH reste à payer.`}
                </p>
              </div>
            </div>
          </div>
