...
export default function ListeFormateurs() {
  ...
  const filteredFormateurs = formateurs.filter((e) =>
    `${e.nom ?? ""} ${e.prenom ?? ""}`
      .toLowerCase()
      .includes(search.toLowerCase().trim())
  );

  const indexDernierFormateur = currentPage * formateursParPage;
  const indexPremierFormateur = indexDernierFormateur - formateursParPage;
  const formateursActuels = filteredFormateurs.slice(indexPremierFormateur,indexDernierFormateur);
  const totalPages = Math.ceil(filteredFormateurs.length / formateursParPage);
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
            ...
            <div className="overflow-x-auto">
              <div className="min-w-[800px]">
                <table className="w-full">
                  ...
                  <tbody>
                    {formateursActuels.length === 0 ? (
                      <tr>
                        <td colSpan="7" className="text-center py-6 text-gray-500">
                          Aucun formateur trouvé
                        </td>
                      </tr>
                    ) : (formateursActuels.map((formateur, index) => (
                      <tr key={formateur.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            <span className="text-sm font-medium text-gray-800">{formateur.nom} {formateur.prenom}</span>
                          </div>
                        </td>
            ...
            <div className="px-6 py-4 border-t border-gray-200 flex justify-between items-center flex-wrap gap-4">
              <p className="text-sm text-gray-500 whitespace-nowrap">
                Total: {formateurs.length} formateurs
              </p>
