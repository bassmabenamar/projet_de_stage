    ...
    <div className="flex min-h-screen bg-[#F8FAFC] font-sans text-[#1E293B]">
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
          {/* --- BARRE SUPÉRIEURE : Statistiques & Contrôles --- */}
          <div className="flex flex-col xl:flex-row gap-6 mb-8 items-stretch">
            ...
            >
              <div className="flex gap-12">
                <StatItem label="Semaine du" value="23 - 27 Octobre" />
                <StatItem label="Heures d'enseignement" value="24 / 40" />
                <StatItem label="Salle actuelle" value="Lab 402" />
              </div>
              ...
              <div className="flex items-center gap-3 bg-slate-50 p-2 rounded-2xl">
                ...
                <span className="px-4 font-black text-xs uppercase tracking-widest text-[#002366]">
                  Aujourd’hui
                </span>
            ...
            >
              <div className="relative z-10">
                ...
                <div className="flex items-center gap-2 opacity-80 text-xs font-bold">
                  <MapPin size={14} /> Bloc B, Salle 102
                </div>
              </div>
            ...
            <div className="grid grid-cols-6 border-b border-slate-50 bg-slate-50/30">
              ...
              {['Lun', 'Mar', 'Mer', 'Jeu', 'Ven'].map((day, i) => (
                <div key={day} className="p-6 text-center border-r border-slate-50 last:border-0">
                  ...
                  <span className={`text-xl font-black ${i === 1 ? 'text-[#002366]' : 'text-slate-400'}`}>
                    {23 + i}
                  </span>

                  {i === 1 && (
                    <div className="w-8 h-1 bg-[#002366] mx-auto mt-2 rounded-full" />
                  )}
                </div>
              ))}
            </div>
              ...
              {['08:00', '09:30', '11:00', '12:30', '14:00'].map((time, idx) => (
                <div key={time} className="grid grid-cols-6 border-b border-slate-50 last:border-0 group">
                  ...
                  <div className="p-8 border-r border-slate-50 flex items-start justify-center">
                    <span className="text-xs font-black text-slate-300 group-hover:text-[#002366] transition-colors">
                      {time}
                    </span>
                  </div>
                  ...
                  <div className="p-3 border-r border-slate-50 relative bg-slate-50/10">
                    {idx === 1 && (
                      <LectureCard 
                        title="Laboratoire de Chimie Organique" 
                        code="CHE-402" 
                        room="402" 
                        type="active" 
                        students="28" 
                    ...
                    {idx === 4 && (
                      <LectureCard 
                        title="Séminaire des Professeurs" 
                        code="GEN-100" 
                        room="Auditorium" 
                        type="purple" 
                      />
          ...
          <div className="grid grid-cols-12 gap-6">
            ...
            <div className="col-span-12 lg:col-span-5 bg-white rounded-[32px] p-8 shadow-sm border border-slate-100">
              <div className="flex justify-between items-center mb-6">
                <h3 className="font-black text-[#002366]">Supports de cours</h3>
              ...
              <div className="space-y-4">
                <MaterialItem 
                  title="Thermodynamique_v2.pdf" 
                  subtitle="Partagé avec Physique Avancée" 
                  type="pdf" 
                />

                <MaterialItem 
                  title="Composés_Organiques_Lab.pptx" 
                  subtitle="Prévu pour le Lab 402" 
                  type="ppt" 
                />
              </div>
            </div>
...
const StatItem = ({ label, value }) => (
  <div>
    ...
    <p className="text-[15px] font-black text-[#002366]">
      {value}
    </p>
  </div>
);
...
const LectureCard = ({ title, code, room, type, students }) => {
  ...
  return (
    ...
    >
      {type === 'active' && (
        <div className="absolute top-0 left-0 bg-orange-500 text-white text-[8px] font-black px-2 py-0.5 rounded-br-lg uppercase">
          En cours
        </div>
      )}

      <div className="flex justify-between items-start mb-2">
        <span className={`text-[10px] font-black uppercase tracking-widest ${type === 'dark' ? 'text-white/60' : 'text-slate-400'}`}>
          {code}
        </span>
...
const MaterialItem = ({ title, subtitle, type }) => (
  ...
  >
    ...
    <div className="flex-1">
      <h5 className="text-sm font-black text-[#002366]">
        {title}
      </h5>
